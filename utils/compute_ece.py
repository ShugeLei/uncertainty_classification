import numpy as np
from utils.binning_methods import BinEqualMass, BinEqualWidth
...
class CalibrationMetric():
    ...
    def predict_top_label(self, fx, y):
        """Compute confidence scores and correctness of predicted labels.
            ...
            Returns:
                ...
                hits: np.ndarray of shape [N, 1] denoting whether or not top label is a correct prediction or not.
        """
    ...
    def compute_error(self, fx, y):
        ...
        if np.max(fx) > 1.:
            raise ValueError("Maximum score of {} is greater than 1.".format(np.max(fx)))
        if np.min(fx) < 0.:
            raise ValueError("Minimum score of {} is less than 0.".format(np.min(fx)))

        if np.max(y) > 1.:
            raise ValueError("Maximum label of {} is greater than 1.".format(np.max(y)))
        if np.min(y) < 0.:
        ...
        if self.ce_type in ["ew_ece_bin", "em_ece_bin"]:
            ...
            bin_fx_y = self.bin_mean(fx, y, self.num_bins, bin_indices)
            ...
            calibration_error, bin_fx_y = self._compute_error_monotonic_sweep(fx, y)
        else:
            raise NotImplementedError("Calibration error {} not supported.".format(self.ce_type))

        return calibration_error, bin_fx_y
    ...
    def _compute_error_all_binned(self, binned_fx, binned_y, bin_sizes):
        ...
        for k in range(num_classes):
            ce_sum += ce[:, k].sum()
        ece = pow(ce_sum / (num_examples*num_classes), 1. / self.norm)
        return ece
    ...
    def _compute_error_label_binned(self, fx, binned_y, bin_indices):
        ...
        for k in range(num_classes):
            for i in range(num_examples):
                ce_sum += pow(np.abs(fx[i, k] - binned_y[bin_indices[i, k], k]), self.norm)
    ...
    def bin_mean(self, fx, y, n_bins, bins):
        ...
        for i in range(n_bins):
            ...
            if any(cur):
                ...
                bin_fx.append(0.0)
                bin_y.append(0.0)
        bin_fx_y = np.stack([np.array(bin_fx), np.array(bin_y)], axis=0)
        return bin_fx_y
    ...
    def _compute_error_monotonic_sweep(self, fx, y):
        ...
        bin_fx_y = self.bin_mean(fx, y, n_bins, bins)
        return ece, bin_fx_y
